...
> 최종 갱신: 2026-07-15 · 부동산 매물 검증 + 방문 예약/정산 백엔드 (포트폴리오)
...
| **6차** | Redis 인기 매물 조회수/캐싱(원자 Lua·writeback·트렌딩 감쇠), 관측성(Prometheus/Grafana/Sentry), k6 부하, Docker/CI | ✅ 구현 완료 |

**규모**: 프로덕션 Java 219개 파일, 테스트 146개(@Test). 6차 신규(view+popular+metrics) 17개 파일 · 테스트 15개.
...
- **관측성(6차)**: Micrometer → Prometheus · Grafana · Sentry · **Docker 멀티스테이지 이미지 + docker-compose 전 스택 + GitHub Actions CI**
...
## 6차 인기 매물 조회수·캐싱 + 관측성/배포 (구현 완료)

설계: `specs/006-ops-performance/` · 기능(조회수·인기)을 축으로 관측성·부하·배포를 검증/계측 수단으로 통합(세로 슬라이스).

### Phase 1 — 조회수 카운팅
- 상세 조회 시 **단일 Lua**로 dedup(`SET NX EX`)+`HINCRBY view:pending`+`ZINCRBY property:popular` 원자 실행(부분성공 차단, best-effort)
- **writeback**: `EXISTS→RENAME view:pending→view:flushing` 원자 배출 → 단일 tx `UPDATE view_count += delta` → 커밋 후 DEL(**유실 0**, 단일 인스턴스 전제)
- `getDetail`→`PropertyDetailResult(countablePublicAccess)`: **ACTIVE 공개표현만** 집계. viewerKey=인증 `u:{id}`/비인증 `ip`(trust-proxy 시만 XFF)

### Phase 2 — 인기 랭킹 + cache-aside
- `GET /api/properties/popular`: 트렌딩 Top-N. **cache-aside 단일 키**(`popular:list`) — miss 시 over-fetch → **DB ACTIVE 필터(제외 권위)** → ZSET 순서 복원 → Top-50 캐시. Redis 장애 시 DB 폴백
- 상세 cache-aside: **역할별 읽기**(anonymous·USER=cache-first / REALTOR·ADMIN=우회→DB). ACTIVE 이탈 정리는 `afterCommit` best-effort(`ZREM`+`DEL`)
- **트렌딩 일 감쇠**: `ZUNIONSTORE WEIGHTS`(×0.5) + 임계 제거. ZSET stale은 감쇠/DB필터로 자연 정리(2계층 정합성)

### Phase 3 — 관측성
- Micrometer→Prometheus(`/actuator/prometheus`, permitAll 무인증) + 커스텀 메트릭(`cache.requests`·`view.dedup.skip`·`view.flush.delta`)
- Grafana provisioning(대시보드) + Sentry(빈 DSN=no-op)
- 발견: `management.prometheus.metrics.export.enabled: true` 명시 필요(폴백 off 판정). 관리 포트 분리는 자식 컨텍스트 보안 미적용으로 미채택

### Phase 4 — 부하(k6)
- `loadtest/k6`: 상세·인기·전문검색 시나리오 + `docs/load-test-results.md §6` 측정 계획. 실측 수치는 TBD(원칙 VII, 커스텀 메트릭으로 산출)

### Phase 5 — 배포/마감
- 멀티스테이지 `Dockerfile`(JDK21 build→JRE21) + docker-compose 전 스택(app+mysql+redis+es+prometheus+grafana, monitoring profile) + GitHub Actions CI(build+Testcontainers+이미지 빌드)

**핵심**: 조회수는 **근사 카운터**(유실 0 지향, 크래시 창 허용)로 정합성 핵심 도메인(예약/정산)과 구분. 인기 목록 정확성은 **DB ACTIVE 필터가 권위**, ZSET/캐시는 best-effort 트렌딩 신호.
