...
class PriceStandardBatchServiceTest {
    ...
    void setUp() {
        ...
        service = new PriceStandardBatchService(realEstateClient, batchJobRepository, properties);
    }
    ...
    void partialFailure() {
        when(realEstateClient.fetch(eq("11680"), any())).thenReturn(List.of(
                new OfficetelRentTransaction("11680", DealType.JEONSE, 220_000_000L, 0, new BigDecimal("29.87"), 2026, 4)));
    ...
    void allSuccess() {
        when(realEstateClient.fetch(any(), any())).thenReturn(List.of(
                new OfficetelRentTransaction("11680", DealType.MONTHLY_RENT, 10_000_000L, 700_000L,
