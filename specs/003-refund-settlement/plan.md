...
**① 잠금(순서 고정) → ② 검증 → ③ 변경** 3단계로 분리. 모든 잠금을 먼저 획득한 뒤 검증한다(검증이 잠금 사이에 끼지 않음, 리뷰 P0-2).

**① 잠금 (Payment → Reservation → VisitSlot, 리뷰 P0-4)**
1. `paymentRepository.findByReservationIdForUpdate(reservationId)`(PESSIMISTIC_WRITE) — reservation 을 먼저 잠그지 않음
2. `Reservation` 잠금
3. `VisitSlot` 잠금

**② 검증 (변경 전, 모든 잠금 확보 후)**
4. **멱등**: reservation 이미 CANCELLED → 현재 상태 반환
5. 소유자 검증(reservation.userId==auth, 아니면 403)
6. 상태 검증: `CONFIRMED` 아니면 409. **주입된 `Clock`** 으로 `slot.start_time <= now+24h` 이면 409(취소 불가)

**③ 변경 (단일 트랜잭션)**
7. `Refund` 생성(refund_amount=payment.amount, refunded_at=now), `Payment.refund()`(PAID→REFUNDED, paidAt 유지), Reservation `cancel()`, VisitSlot `reopen()`(RESERVED→OPEN)
...
0. **대상 realtor 집합** = 당월 paidAt 결제 realtor ∪ 당월 refundedAt 환불 realtor ∪ **전월 carry_over_out > 0 인 realtor**(당월 결제/환불이 없어도 이월 이어가야 함, 리뷰 P0-1)
1. 각 realtor: 당월 결제 합(paidAt∈월, status∈{PAID,REFUNDED}) / 환불 합(refundedAt∈월) 집계(없으면 0)
2. 각 realtor: 전월 settlement.carry_over_out → carry_over_in
