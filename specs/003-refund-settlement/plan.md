...
4. `Settlement(PENDING)` upsert — **UNIQUE(realtor_id, month)**:
   - **이후 월 정산이 존재하면 재계산 금지(409)** (carry_over 연쇄 회피, 단순 정책)
   - 대상 월 PENDING & 이후 월 없음 → 재계산 갱신
   - CONFIRMED/PAID → skip
5. 결과 `{ month, createdCount, updatedCount, skippedCount }` 반환.
