...
- `POST /visit-slots/{slotId}/reservations`(토큰 보유자): Reservation `PENDING_PAYMENT` + Payment `READY` 동시 생성(토큰 소유자 검증).
- `PATCH /payments/{id}/confirmation`(READY→PAID): §2.4 확정 트랜잭션 실행 → Reservation `CONFIRMED`, slot `RESERVED`, 토큰 삭제. 멱등.
