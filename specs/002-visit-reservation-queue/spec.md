...
### 2.1 슬롯당 1명 확정
- `UNIQUE(active_reservation_key)`(status IN PENDING_PAYMENT/CONFIRMED 일 때 visit_slot_id) 를 최종 방어선으로 둔다. 만료/실패 후 같은 슬롯 재예약(이력 다건)을 허용하면서 활성 1건을 보장(§data-model).
...
- 대기열 key: `waiting:visit-slot:{slotId}` (ZSet, member=userId, **score=전역 시퀀스 `INCR waiting:seq`** — 강한 FIFO, ms 충돌 없음)
...
- 발급은 **원자적 연산**으로만 수행한다. 순서가 중요하다 — 토큰에 넣을 userId 는 ZSET 에서 pop 해야 알 수 있으므로 `SET NX` 를 먼저 하면 안 된다. 택1:
  - **(A) Lua Script** (권장): 단일 스크립트로 아래를 원자 실행.
    ...
    2. `ZPOPMIN waiting:visit-slot:{slotId}` 로 선두 userId pop. 큐가 비었으면 종료.
...
- 대기 순번 조회: `ZRANK waiting:visit-slot:{slotId} {userId}` (+1).
...
| HELD | 결제 확정(PAID) | RESERVED | MySQL OPEN→RESERVED, `UNIQUE(active_reservation_key)` 최종 방어 |
...
4. 동일 slot 활성 예약 없음 (`UNIQUE(active_reservation_key)` 보장)
- **확정 처리**: 토큰(Redis) 확인 → **단일 MySQL 트랜잭션**에서 Reservation `CONFIRMED` + Payment `PAID` + `visit_slot.status OPEN→RESERVED`(`UNIQUE(active_reservation_key)` 최종 방어) → 커밋 후 Redis 토큰 삭제.
...
- **활성 예약 유일성**: 최종 방어는 `UNIQUE(active_reservation_key)`(visit_slot_id 직접 UNIQUE 아님 — 만료/실패 후 재예약 이력 다건 허용).
- **예약 생성 재시도**: `active_reservation_key` 충돌 시 직전 사용자의 만료 PENDING 을 정리 후 **1회 재시도**(리뷰-4).
- **대기열 순서**: score = `INCR waiting:seq`(전역 단조) → ms 충돌 없는 강한 FIFO.
- **실패 락**: failure 는 `Payment→Reservation` 잠금 후 재확인(PAID면 skip). **슬롯 마감**: CLOSED 는 짧은 조건부 update 로 커밋 + Redis 정리, PENDING 정리는 **별도 tx·공통 락 순서**(락 역순 방지)(리뷰-4).
