...
- **9090 `/actuator/prometheus` 스크레이프(P3)**: `management.server.port=9090` 분리·호스트 미매핑. 테스트는 `management.server.port=0` + `@LocalManagementPort` 로 검증(운영 포트 고정 아님, P3-테스트). Authorization 없이 `/actuator/prometheus` → **200**(actuator 전용 `@Order(0)` permit 체인). 대시보드: 캐시 hit-ratio, dedup 비율, writeback 델타, 엔드포인트 p95.
